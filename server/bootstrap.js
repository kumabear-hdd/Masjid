/**
 * Idempotent bootstrap: ensure admin + sample schedules exist.
 * Safe to call on every server start (production-friendly).
 */
...
function bootstrap() {
  ...
  if (count === 0) {
    ...
    tx(samples);
    console.log(`Bootstrap: ${samples.length} jadwal contoh`);
  }
